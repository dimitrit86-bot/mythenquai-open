...
async function submitBet(){
  ...
  try{
    ...
    const dl='https://wpmyuzpcraduhaybjvmb.supabase.co/functions/v1/download-code?code='+encodeURIComponent(accessCode)+'&name='+encodeURIComponent(name)+'&side='+encodeURIComponent(side==='D'?'Dimitri':'Florian')+'&amount='+encodeURIComponent(amount);
    $('entryMsg').innerHTML='<div class="person"><span class="ok"><b>Einsatz vorgemerkt ✓</b></span><br>'+esc(name)+' · '+(side==='D'?'Dimitri':'Florian')+' · '+CHF(amount)+'<hr><div class="note">DEIN PERSÖNLICHER CODE</div><div class="codebox" style="margin:8px 0;font-size:28px;font-weight:900">'+esc(accessCode)+'</div><p><b>Wichtig: Speichere diesen Code jetzt.</b><br><span class="note">Du brauchst ihn später, um deine persönliche Quote aufzurufen und deine Wette verbindlich zu bestätigen oder auszusteigen.</span></p><div class="row" style="margin-top:12px"><a class="btn primary" id="downloadCode" href="'+dl+'">CODE SPEICHERN</a><button class="btn" id="copyCode">CODE KOPIEREN</button></div><p class="note">Tipp: Am einfachsten den Code herunterladen oder zusätzlich einen Screenshot machen.</p><div class="row" style="margin-top:12px"><a class="btn primary" href="game.html?v=16">🎾 MATCH SIMULIEREN</a><button class="btn" id="again">WEITEREN EINSATZ ERFASSEN</button></div></div>';
    $('copyCode').onclick=async()=>{try{await navigator.clipboard.writeText(accessCode);$('copyCode').textContent='KOPIERT ✓';}catch(e){prompt('Code kopieren',accessCode)}};
