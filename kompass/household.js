...
let auth=null,current=null,raw=null,pending=false,busy=null,blocked=false,timer,cryptoKey=null,cacheJob=Promise.resolve(),cacheSeq=0,status='Nicht angemeldet',started=false,profiles=[],activation=new URLSearchParams(location.hash.slice(1)).get('activate');
...
async function cacheState(){
 if(!current||!cryptoKey)return;const seq=++cacheSeq,record={id:current.id,name:current.name,revision:current.revision,raw,pending};
...
async function choose(id){
 ...
 current=p;raw=JSON.stringify(window.NK.validateState(p.state,window.NK_DATA.nutrients.map(n=>n.key)));pending=false;blocked=false;safeStore(localStorage,LAST,p.id);await cacheState();dlg.close();await openApp();
}
...
async function logout(){if(pending)await flush();await api('logout');auth=null;cryptoKey=null;current=null;raw=null;pending=false;blocked=false;cacheSeq++;await cacheJob.catch(()=>{});sessionStorage.removeItem(SESSION);localStorage.removeItem(SESSION);localStorage.removeItem(CACHE);window.NK_APP?.clearState();dlg.close();showGate();}
...
async function boot(){
 ...
 try{const saved=sessionStorage.getItem(SESSION)||localStorage.getItem(SESSION);if(saved)auth=JSON.parse(saved);if(auth&&Date.parse(auth.expires)>Date.now()){
  cryptoKey=await importKey(auth.key);try{await afterLogin();return;}catch(e){if(e.status===401||e.status===403){auth=null;sessionStorage.removeItem(SESSION);localStorage.removeItem(SESSION);showGate('login','Sitzung abgelaufen. Bitte erneut anmelden. Ungesendete Änderungen bleiben verschlüsselt erhalten.');return;}const c=await readCache();if(c){current={id:c.id,name:c.name,revision:c.revision};raw=c.raw;pending=c.pending;await openApp();setStatus('Offline · zuletzt gespeicherte Fassung');return;}throw e;}
